...
public class ProfileSearch extends AppCompatActivity {
    ...
    public void onSearchButtonClickWhynowork(View view)
    {
        getUserChildKey();
    }

    public void getUserChildKey( )
    {
        Log.d("Jason", "Fat bitch ");
        friendsListdb.orderByChild("/" + currentUser + "/").equalTo(currentUser).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChildren())
                {
                    Iterator<DataSnapshot> friendID = dataSnapshot.getChildren().iterator();

                    while (friendID.hasNext())
                    {
                        String friendIDStr = friendID.next().child("frienduid").getValue().toString();
                        Log.d("Jason", "friendID: " + friendIDStr);
                    }
                }
            }
    ...
    private void findUserInDatabase(String friendKey){
        DatabaseReference listOfFriends = friendsListdb.child(friendKey + "/friendslist");
        listOfFriends.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
